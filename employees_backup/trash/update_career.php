<?php
  ...
  if(!file_exists($dir)){
	...
	$attach = '';
	if(!empty($_FILES['career_attach']['tmp_name'][0])){
		$sql = "SELECT attachments FROM ".$cid."_emp_career WHERE id = '".$_REQUEST['id']."'";
		...
		foreach($_FILES['career_attach']['tmp_name'] as $k=>$v){
			$filename = $_FILES['career_attach']['name'][$k];	
	...
	$sql = "INSERT INTO ".$cid."_emp_career (";
