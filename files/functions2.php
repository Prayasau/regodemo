<?php
	...
	function getEntityData($entity){
		global $dbc;
		$edata = array();
		if($res = $dbc->query("SELECT ".$_SESSION['rego']['lang']."_addr_detail, ".$_SESSION['rego']['lang']."_compname, comp_phone, comp_fax, tax_id, revenu_branch, dig_signature, digi_signature, dig_stamp, digi_stamp, logofile, sso_account, sso_codes, banks, en_address, th_address FROM ".$_SESSION['rego']['cid']."_entities_data WHERE ref = '".$entity."'")){
	...
	function checkEmployeesForPayroll($cid){
		...
		$sql = "SELECT emp_id, title, firstname, en_name, th_name, lastname, base_salary, joining_date, emp_status, team FROM ".$cid."_employees WHERE emp_status < 4 ORDER BY emp_id ASC";
	...
	function getEmployeeStatus($cid){
		...
		$sql = "SELECT emp_id, title, emp_status, idcard_nr, base_salary, joining_date, en_name, firstname, lastname FROM ".$cid."_employees WHERE emp_group = 's'";
		if($res = $dbc->query($sql)){
			while($row = $res->fetch_assoc()){
				if($row['title'] == '' ||
					$row['firstname'] == '' || 
