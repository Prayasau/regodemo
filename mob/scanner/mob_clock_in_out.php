...
<html>
  ...
  <body style="background:#006666">
		...
		<div style="position:absolute; top:55px; bottom:0; left:0; right:0; padding:10px; text-align:center">
			...
			<p style="padding:10px 0 2px; font-size:20px;"><?=$title[$data['title']].' '.$data[$lang.'_name']?><br>
      <?=$positions[$data['position']]?></p>
      ...
      <div id="qr-result" hidden="">
        <span id="outputData"></span>
			...
			<h1 style="padding-bottom:0">Your location</h1> 
			<p id=loc></p>
			<? //=var_dump($scanloc)?>
			<div style="position:absolute; bottom:20px; left:0; right:0; text-align:center">
				<button style="border-radius:3px" onClick="window.location.reload()" type="button" class="btn btn-success"><i class="fa fa-refresh"></i>&nbsp; Reload<? //=$lng['Submit']?></button>
			</div>
	...
	$(document).ready(function() {
			...
			qrcode.callback = res => {
				if (res) {
					...
					if(res in myqrcode){
						...
						outputData.innerText = 'Wrong QR code';
						scanning = false;
					...
					if(parseInt(distance) > parseInt(perimeter)){
						outputData.innerText = 'You are out of range';
						scanning = false;
						video.srcObject.getTracks().forEach(track => {
							track.stop();
						});
					...
					$.ajax({
						...
						success: function(response){
							...
							if(selfie == 1){
								...
								outputData.innerText = employee['en_name'] + '\n' + datetime + '\nTime registered successfully';
								qrResult.hidden = false;
			...
			function showLocation(loc) { 
				...
				'Accuracy : <b>' + Globalize.format(loc.coords.accuracy,'n0') + ' m.' + '</b><br>' +
				'Distance : <b>' + parseInt(distance) + ' m.<br>' + scanlocation + '</b>';
			} 
