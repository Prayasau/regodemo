<?
	...
	$sql = "SELECT * FROM rego_users WHERE super_admin = 'admin' ORDER by LENGTH(user_id) ASC, user_id ASC";
	$res = $dba->query($sql);
	if(mysqli_error($dba)){
...
?>
	...
	<h2><i class="fa fa-table"></i>&nbsp;&nbsp;<?=$lng['Xray users']?></h2>
	<div class="main">
         ...
         <fieldset disabled>
            ...
            <tbody>
							...
							<tr>
								...
								<td>
									<input name="customer_registration[delete]" type="hidden" value="0"  />
									<label><input disabled type="checkbox" name="customer_registration[delete]" value="1" class="cUser checkbox style-0"><span><?=$lng['Delete']?></span></label>
								</td>
								<td colspan="11"></td>
							</tr>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$(document).on('submit','form#userForm', function(e){
				...
				if(!file){
					$.ajax({
						url: "ajax/save_admin_xrayuser_data.php",
						type: 'POST',
						data: data,
						async: false,
						cache: false,
			...
			$(document).on("click", "#add_user", function(){
				...
				$.ajax({
					url: "ajax/get_admin_user_ID_xray.php",
					success:function(data){
						//$('#dump').html(data);
						$("#user_id").val(data);
					},
