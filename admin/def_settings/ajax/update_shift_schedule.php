<?
	...
	//  if no scan = 1 empty the ss data cycle data 
	$id = $_REQUEST['shift_schedule']['team_id'];

	// static ss data for no scan  
	...
	if($_REQUEST['shift_schedule']['noscan'] == '1')
	{
		...
		$sql = "UPDATE rego_default_shiftplans SET name = '".$dba->real_escape_string($_REQUEST['shift_schedule']['name'])."',description = '".$dba->real_escape_string($_REQUEST['shift_schedule']['description'])."',ss_data = '".$dba->real_escape_string($ssData)."', dates = '', cycle_details= '', wh_code='', wkd='' WHERE id ='".$id."' ";
	}
	else
	{
		// for 2 scan and 4 scan 
		$sql = "UPDATE rego_default_shiftplans SET name = '".$dba->real_escape_string($_REQUEST['shift_schedule']['name'])."',description = '".$dba->real_escape_string($_REQUEST['shift_schedule']['description'])."',ss_data = '".$dba->real_escape_string(serialize($_REQUEST['shift_schedule']))."' WHERE id ='".$id."' ";
	}
