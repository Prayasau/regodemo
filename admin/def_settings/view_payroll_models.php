<?
	...
	$getPayrollModels = array();
	$sqlv = "SELECT * FROM rego_payroll_models WHERE apply = '1' AND id='".$_GET['mid']."'";
	if($resv = $dba->query($sqlv)){
		while($rowv = $resv->fetch_assoc()){
			$getPayrollModels[] = $rowv;
		}
	}
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$(document).on('click', '.editItem', function(){
			...
			$.ajax({
				type : 'post',
				url  : 'def_settings/ajax/get_payroll_model_data.php',
				data : {id:mid},
				dataType: 'json',
