<?
	...
	$data = array();
	if($res = $dbx->query("SELECT * FROM rego_support_desk  WHERE ticket = '".$_REQUEST["id"]."'")){
	...
	$message = array();
	if($res = $dbx->query("SELECT * FROM rego_support_tickets  WHERE ticket = '".$_REQUEST["id"]."' ORDER BY date DESC")){
		while($row = $res->fetch_assoc()){
