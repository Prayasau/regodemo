<?
	...
	if($_REQUEST['upgrade_period'] == 'fullyear'){
		...
		$price = $price_table[$ver]['price_year'];//$_REQUEST['new_price'];
		...
		$price = $price_table[$ver]['price_year'] / 365 * $_REQUEST['days_left'];
		$discount = $price_table[$ver]['discount'] / 365 * $_REQUEST['days_left'];
		$price_rem_period = $_REQUEST['prev_price'] / 365 * $_REQUEST['days_left'];
		$price_upgrade = $price - $price_rem_period - $discount;
	}else{
	...
	unset($_SESSION['rego']['new']);
	$_SESSION['rego']['upgrade'] = $data;
