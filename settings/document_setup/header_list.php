...
<h2 style="padding-right:60px">
	<i class="fa fa-file-text"></i>&nbsp; <?=$lng['Header templates']?>
</h2>
		...
		<div style="padding:0 0 0 20px" id="dump"></div>
			...
			<div class="row">
				...
				<div class="col-md-2">
					...
					<a type="button" onclick="window.location.href='index.php?mn=310&hid=n';" class="btn btn-primary btn-fr">
						<i class="fa fa-plus pr-2"></i> <?=$lng['New'].' '.$lng['Header']?>
					</a>
				</div>
			</div>
				...
				<tbody>
					<?if(isset($data) && is_array($data)){ 
						...
						<tr>
							...
							<td onclick="rowClick(<?=$v['id']?>)"><?=$v['company']?></td>
							<td onclick="rowClick(<?=$v['id']?>)"><?=$v['remark']?></td>
							<td><a id="<?=$v['id']?>" onclick="RemoveHeader(this)"><i class="fa fa-trash fa-lg"></i></a></td>		
						</tr>
					<? } } ?>
				</tbody>
			</table>
	...
	<script type="text/javascript">
		...
		function RemoveHeader(that){
			var id = that.id; 
			$.ajax({
				url:"document_setup/ajax/remove_header.php",
				type: 'POST',
				data: {id: id},
				success: function(response){
					...
					$("body").overhang({
						type: "success",
						message: '<i class="fa fa-check"></i>&nbsp;&nbsp;Header removed successfully',
						duration: 3,
						callback: function(v){
							location.reload();
						}
		...
		function rowClick(Hid) {
			window.location.href='index.php?mn=310&hid='+Hid;
		}
