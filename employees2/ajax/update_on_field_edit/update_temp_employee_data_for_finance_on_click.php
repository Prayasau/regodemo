<?
	...
	//print_r($rego_settings['bank_codes']);exit;
	...
	if($updateValue=='bank_account' || $updateValue=='modify_tax' || $updateValue=='savings' || 
	    $updateValue=='legal_execution' || $updateValue=='kor_yor_sor'){
		...
		$sqlaLL1 = "SELECT * FROM ".$cid."_temp_log_history WHERE emp_id = '".$value['emp_id']."' AND field = '".$emp_db[$updateValue]."'";
			if($resaLL1 = $dbc->query($sqlaLL1))
			{
				if($rowaLL1 = $resaLL1->fetch_assoc())
				{
					...
					//echo "UPDATE  ".$cid."_temp_log_history SET   date = '".$dateUpdate."' ,  prev = '".$dbc->real_escape_string($value[$updateValue])."', new = '".$dbc->real_escape_string($postValue)."' WHERE emp_id = '".$value['emp_id']."' AND field = '".$emp_db[$updateValue]."'";
				}
				else
				{
					// insert 
	...
	$dbc->query($sql1);
	if($updateValue=='bank_name'){
	    $sql= "UPDATE ".$_SESSION['rego']['cid']."_temp_employee_data SET  bank_code = '".$postValue."' WHERE id = '".$_REQUEST['rowId']."'";
	    $dbc->query($sql);
	}
