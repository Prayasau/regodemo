<?
	...
	// check for username options 

	if($updateValue ==  'username_option')
	{
		// set peronal and work email according to that 

		$fetch_old_data = $dbc->query("SELECT * FROM ".$cid."_temp_employee_data ");
		...
		if($fetch_old_data->num_rows > 0){ 
			while ($row = $fetch_old_data->fetch_assoc()) {
				...
				if($postValue == '1')
				{
					$dbc->query("UPDATE ".$_SESSION['rego']['cid']."_temp_employee_data SET username = '".$row['personal_email']."' WHERE emp_id =  '".$row['emp_id']."'");
				}
				else if($postValue == '2')
				{
					$dbc->query("UPDATE ".$_SESSION['rego']['cid']."_temp_employee_data SET username = '".$row['work_email']."' WHERE emp_id =  '".$row['emp_id']."'");
				}
				else if($postValue == '3')
				{
					$dbc->query("UPDATE ".$_SESSION['rego']['cid']."_temp_employee_data SET username = '' WHERE emp_id =  '".$row['emp_id']."'");
				}
			}
		}
