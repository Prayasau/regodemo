							...
							<table  id="datatables25" class="dataTable hoverable selectable nowrap" >
								...
								<? if(isset($alltempdata) && is_array($alltempdata)){ 
										foreach ($alltempdata as $key => $value) { ?>
											...
											<tr data-id="<?=$value['emp_id']?>">
												<td><span id="rowIdDatatableSpan" style="display: none;"><?=$value['id']?></span><?=$value['emp_id']?></td>
												<td><?=$value['Contract type'];?></td>
												<td><?=$value['Calculation base']?></th>
            									<td><?=$value['Bank code']?></th>
            									<td><?=$value['Bank name']?></th>
            									<td><?=$value['Bank branch']?></th>
            									<td><?=$value['Bank account no.']?></th>
            									<td><?=$value['Bank account name']?></th>
            									<td><?=$value['Payment type']?></th>
            									<td><?=$value['Accounting code']?></th>
            									<td><?=$value['Groups']?></th>
            									<td><?=$value['Tax calculation method']?></th>
            									<td><?=$value['Calculate tax']?></th>
            									<td><?=$value['Tax Residency Status']?></th>
            									<td><?=$value['Income Section']?></th>
            									<td><?=$value['Modify Tax amount']?></th>
            									<td><?=$value['Calculate SSO']?></th>
            									<td><?=$value['SSO paid by']?></th>
            									<td><?=$value['Government house banking']?></th>
            									<td><?=$value['Savings']?></th>
            									<td><?=$value['Legal execution deduction']?></th>
            									<td><?=$value['Kor.Yor.Sor (Student loan)']?></th>
