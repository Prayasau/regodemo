<?
	...
	if($res = $dbc->query("SELECT month FROM ".$_SESSION['rego']['payroll_dbase']." WHERE paid = 'C' ORDER BY month ASC")){
		while($row = $res->fetch_assoc()){
	...
	$getCurrentMon = date('n');

	$myMonlist = array();
	foreach ($months as $key => $value) {
		if($key < $getCurrentMon){
			$myMonlist[$key] = $value; 
		}else{
			break;
		}
	...
	$his_cols = array();
		...
		$his_cols[] = array('tax_by_company',$lng['Tax by company']);
		$his_cols[] = array('sso_by_company',$lng['SSO by company']);
	...
	<div class="main">
		...
		<div id="showTable" style="display:none">
			...
			<div class="btns-group-left">
				...
				<? if($_GET['type'] == 'add'){ ?>
					<!-- <select id="sMonth" class="button monthFilter btn-fl" onchange="getselectedmonth(this.value);">
			...
			<table id="datatable" class="dataTable attendance">
				<thead class="tac">
					<tr style="line-height:100%">
						...
						<th class="mw" data-sortable="false"><?=$lng['Tax by company']?></th>
						<th class="mw" data-sortable="false"><?=$lng['SSO by company']?></th>
...
.confDelete {
   ...
   <!-- PAGE RELATED PLUGIN(S) -->
	...
	$(document).ready(function() {
		...
		var monthss = <?=json_encode($_SESSION['rego']['cur_month'])?>;
		...
		$("#addEmp").on('change', function(){
			...
			var mOpt = $('#sMonth').val();
			if(mOpt == 'all'){ var NewmonVal = monthss;}else{ var NewmonVal = mOpt;}
			//alert(NewmonVal);
			//alert(ids);

			$.ajax({
				url: ROOT+"payroll/ajax/add_historic_data.php",
				data: {ids: ids, month: NewmonVal},
				success: function(response) {
					//$("#dump").html(response); return false;
					$("#addEmp").val('0');
	...
	function getMissingEmployees(m){
		...
		$.ajax({
			url: "ajax/get_missing_employees_payroll.php",
			data: {m: m},
			dataType: "json",
			success:function(result){
				//$('#dump').html(result); return false;
