<?php
	...
	foreach($emps as $k=>$v){
		...
		$emp_name = $title[$empinfo['title']]. ' '.$empinfo[$lang.'_name'];
		$emp_address = '';
		...
		if($res = $dbc->query("SELECT SUM(gross_income) as gross, SUM(tax_month) as tax, SUM(pvf_employee + psf_employee) as pvf, SUM(social) as sso FROM ".$_SESSION['rego']['payroll_dbase']." WHERE emp_id = '".$k."'")){
	...
	$html .= '
				...
				<div class="field13" style="top:44.6mm;left:22mm">'.$comp_address.'</div>
				...
				<div class="field13" style="top:62.2mm;left:20mm">'.$emp_name.'</div>
	...
	$html .= '
				...
				if($nr < count($emps)){ $html .= '<pagebreak />';	}
}	
	$html .= '</body></html>';
	...
	$mpdf->SetTitle($edata['th_compname'].' ('.strtoupper($cid).') - Withholding Tax Certificates '.$_SESSION['rego']['year_'.$lang]);
	$mpdf->SetDisplayMode('fullpage');
	$mpdf->SetFontSize(12);
	$mpdf->WriteHTML($style,1);
	$mpdf->WriteHTML($html);
	$mpdf->Output($cid.'_Withholding_Tax_Certificates_'.$_SESSION['rego']['year_'.$lang].'.pdf','I');
