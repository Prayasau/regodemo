...
$style = '
	...
	if(!empty($compinfo['logofile'])){
		$header .= '<img style="height:40px;max-width:150px;float:left;margin-right:8px" src="../../'.$compinfo['logofile'].'?'.time().'" />';	
	}
	$header .= '
			...
			<table class="report" border="0">
				...
				<tbody>
					<tr>
						...
						<td rowspan="8" style="width:1px; padding:0; padding-top:5px"><img height="160px" src="../../'.$data['image'].'" /></td>
					</tr>
					<tr>
	...
	$html .= '
			<table class="report" border="0" style="table-layout:fixed">
				...
				<tbody>
					<tr>
						<td colspan="2"><b>'.$lng['Address'].' :</b> '.$data['address1'].'</td>
					...
					<tr>
						<td colspan="3"><b>'.$lng['Current address'].' :</b> '.$data['address2'].'</td>
					</tr>
				</tbody>
			</table>';
					...
					$html .= '
						<td style="padding:0">
							...
							<table class="basicTable" border="0" style="width:100%; border-collapse:collapse">
								...
								<tbody>';
								foreach($data['emergency_contacts'] as $k=>$v){
									$html .= '
									<tr>
										<td>'.$v['name'].'</td>
										<td>'.$v['relation'].'</td>
	...
	$html .= '
			<table class="report" border="0" style="">
				...
				<tbody>
					<tr>
						<td><b>'.$lng['Position'].' :</b> '.$positions[$data['position']].'</td>
						<td><b>'.$lng['Employee status'].' :</b> '.$emp_status[$data['emp_status']].'</td>
						<td><b>'.$lng['Employee type'].' :</b> '.$emp_type[$data['emp_type']].'</td>
					</tr>
					<tr>
						<td><b>'.$lng['Joining date'].' :</b> '.$data['startdate'].'</td>
						<td><b>'.$lng['Probation due date'].' :</b> '.$data['probation_date'].'</td>
						<td><b>'.$lng['Service years'].' :</b> '.getAge($data['startdate']).'</td>
					</tr>
					<tr>
						<td><b>'.$lng['Resign date'].' :</b> '.$data['resign_date'].'</td>
						<td colspan="2"><b>'.$lng['Employee platform'].' :</b> '.$yesno[$data['allow_login']].'</td>
					</tr>
					<tr>
					</tr>
					<tr>
	...
	$html .= '
			<table class="report" border="0" style="table-layout:fixed">
				...
				<tbody>
					...
					<tr>
						<td><b>'.$lng['PVF rate employee'].' :</b> '.$data['pvf_rate_employer'].' %</td>
						<td><b>'.$lng['PVF rate employer'].' :</b> '.$data['pvf_rate_employer'].' %</td>
						<td><b>'.$lng['Modify Tax amount'].' :</b> '.$data['modify_tax'].'</td>
					</tr>
					<tr>
						<td><b>'.$lng['Calculate SSO'].' :</b> '.$yesno[$data['calc_sso']].'</td>
						<td><b>Pension fund :</b> '.$pensionfund[$data['calc_pvf']].'</td>
						<td><b>'.$lng['Calculate Tax'].' :</b> '.$yesno[$data['calc_tax']].'</td>
					</tr>
				</tbody>
			</table>';
