<?php
	...
	if(isset($_REQUEST['prid'])){ // RECALCULATE EMPLOYEE //////////////////////////////////////////////////////
		...
		foreach($_REQUEST as $k => $v){ // save attendance data
			$sql .= $k." = '".$v."', ";
		}
		$sql = substr($sql, 0, -2)." WHERE id = '".$prid."'";
		if(!$dbc->query($sql)){
	...
	if($prid){ // RECALCULATE EMPLOYEE ////////////////////////////////////////////////////////////////////////
		$sql = "SELECT * FROM ".$_SESSION['rego']['payroll_dbase']." WHERE id = '".$prid."'";
	}else{
		$sql = "SELECT * FROM ".$_SESSION['rego']['payroll_dbase']." WHERE entity = '".$_SESSION['rego']['selpr_entities']."' AND emp_group = '".$_SESSION['rego']['emp_group']."' AND month = '".$_SESSION['rego']['cur_month']."' AND paid = 'C' ORDER by LENGTH(emp_id) ASC, emp_id ASC";
		//$sql = "SELECT * FROM ".$_SESSION['rego']['payroll_dbase']." WHERE month = '".$_SESSION['rego']['cur_month']."' AND paid = 'C' AND emp_id = 'DEMO-001'";
	}

	if($res = $dbc->query($sql)){
		while($row = $res->fetch_assoc()){
			//var_dump(preg_match('#^[0-9][0-9]:[0-5][0-9]?$#', $row['ot1h']));
			if(!empty($row['ot1h']) && !preg_match('#^$|[0-9]:[0-5][0-9]?$#', $row['ot1h'])){
				ob_clean(); echo 'ottime1'; exit;
			}
			if(!empty($row['ot15h']) && !preg_match('#^$|[0-9]:[0-5][0-9]?$#', $row['ot15h'])){
				ob_clean(); echo 'ottime15'; exit;
			}
			if(!empty($row['ot2h']) && !preg_match('#^$|[0-9]:[0-5][0-9]?$#', $row['ot2h'])){
				ob_clean(); echo 'ottime2'; exit;
			}
			if(!empty($row['ot3h']) && !preg_match('#^$|[0-9]:[0-5][0-9]?$#', $row['ot3h'])){
				ob_clean(); echo 'ottime3'; exit;
			}
			
			if(!empty($row['absence']) && !preg_match('#^$|[0-9]:[0-5][0-9]?$#', $row['absence'])){
				ob_clean(); echo 'abtime'; exit;
			}
			if(!empty($row['late_early']) && !preg_match('#^$|[0-9]:[0-5][0-9]?$#', $row['late_early'])){
				ob_clean(); echo 'latime'; exit;
			}
			if(!empty($row['leave_wop']) && !preg_match('#^$|[0-9]:[0-5][0-9]?$#', $row['leave_wop'])){
				ob_clean(); echo 'lwtime'; exit;
			}
			...
			if($row['contract_type'] == 'month'){
				...
				$basic_salary = $row['basic_salary'] * 30; 
			}
	...
	// PVF & SSO RATES //////////////////////////////////////////////////////////
			...
			$pvf_comp = $row['pvf_rate_com'];
			
			//$sso_rate_emp = $row['sso_rate_emp']/100;
			...
			//$sso_rate_com = $row['sso_rate_com']/100;
	...
	// OVERTIME //////////////////////////////////////////////////////////////////
			if($row['base_ot_rate'] == 'cal'){
				$ot1 = $hour_rate * decimalHours($row['ot1h']);
				$ot15 = ($hour_rate*1.5) * decimalHours($row['ot15h']);
				$ot2 = ($hour_rate*2) * decimalHours($row['ot2h']);
				$ot3 = ($hour_rate*3) * decimalHours($row['ot3h']);
			}else{
				$ot1 = $row['ot_rate'] * decimalHours($row['ot1h']);
				$ot15 = $row['ot_rate'] * decimalHours($row['ot15h']);
				$ot2 = $row['ot_rate'] * decimalHours($row['ot2h']);
				$ot3 = $row['ot_rate'] * decimalHours($row['ot3h']);
			}
			//$oot = $row['ootb'];
	...
	// FIXED & VARIABLE ALLOWANCES ///////////////////////////////////////////////
			for($i=1;$i<=10;$i++){
				if($worked_days < $workdays){ // fired or started employee
					$fix_allowances[$i] =(($row['fix_allow_'.$i] / $workdays) * $worked_days);
				}else{
					$fix_allowances[$i] = $row['fix_allow_'.$i];
				}
			}
			for($i=1;$i<=10;$i++){
	...
	// ABSENCE - LEAVE WOP - LATE EARLY //////////////////////////////////////////
			$absence_b = (decimalHours($row['absence']) / $dayhours) * $day_rate;
			$leave_wop_b = (decimalHours($row['leave_wop']) / $dayhours) * $day_rate;
			$late_early_b = (decimalHours($row['late_early']) / $dayhours) * $day_rate;
	...
	// PVF MONTH /////////////////////////////////////////////////////////////////
			...
			if($row['calc_pvf']){
				...
				$pvf_employee = round($pvf_employee,2);
				$pvf_employer = round($pvf_employer,2);
				/*if($pvf_rate >100){}
					$pvf_employee = $pvf_rate; 
				}else{
					$pvf_employee = ($this_salary * $pvf_rate)/100;
				} 
				if($pvf_com >100){}
					$pvf_employer = $pvf_comp; 
				}else{
					$pvf_employer = ($this_salary * $pvf_comp)/100;
				}*/ 
	...
	// SSO MONTH /////////////////////////////////////////////////////////////////
			...
			if($row['calc_sso']){
				...
				$sso_emp = (($this_salary + $tot_fix_all + $row['remaining_salary']) - ($row['leave_wop_b'] + $row['absence_b'] + $row['late_early_b'])) * $sso_rate_emp;
				...
				$sso_com = (($this_salary + $tot_fix_all + $row['remaining_salary']) - ($row['leave_wop_b'] + $row['absence_b'] + $row['late_early_b'])) * $sso_rate_com;
			...
			$msg .= '<br>&nbsp;-&nbsp;SSO employee = ('.$pr_settings['sso_rate_emp'].'%) '.number_format($sso_emp,2);
	...
	// BASIC CALCULATIONS ////////////////////////////////////////////////////////
			...
			$gross_income_month = $this_salary + $tot_fix_all + $tot_ot + $tot_var_all + $tot_other_var_income;

			// $socialNew = ($gross_income_month - $row['leave_wop_b']);
			// $ssoCalVal = (($socialNew*$row['sso_rate_emp'])/100);
			// $sso_emp   = $ssoCalVal; 
			// $sso_com   = $ssoCalVal; 
	...
	// TAX CALCULATIONS WHOLE YEAR ///////////////////////////////////////////////
			//var_dump($pvf_year); exit;
			if($row['sso_by']){$sso = $sso_year;}else{$sso = 0;}
			$cam_tax = calculateAnualTax($cam_fix, $row['calc_base'], $sso, $pvf_year);
			$cam_tax_previous = calculateAnualTax($cam_fix_previous, $row['calc_base'], $sso, $pvf_year);
			$cam_tax_variable = calculateAnualTax($cam_fix_variable, $row['calc_base'], $sso, $pvf_year);
			$cam_tax = round($cam_tax,2);
			$cam_tax_previous = round($cam_tax_previous,2);
			$cam_tax_variable = round($cam_tax_variable,2);

			$acm_tax = calculateAnualTax($acm_fix, $row['calc_base'], $sso, $pvf_year);
			$acm_tax_previous = calculateAnualTax($acm_fix_previous, $row['calc_base'], $sso, $pvf_year);
			$acm_tax_variable = calculateAnualTax($acm_fix_variable, $row['calc_base'], $sso, $pvf_year);
			$acm_tax = round($acm_tax,2);
			$acm_tax_previous = round($acm_tax_previous,2);
			$acm_tax_variable = round($acm_tax_variable,2);

			//var_dump($cam_tax);

			$tax_ytd = calculateAnualTax($ytd_income, $row['calc_base'], $sso, $pvf_year);
			$tax_ytd = round($tax_ytd,2);
	...
	// TAX SUMMARY ///////////////////////////////////////////////////////////////
			...
			if($row['calc_tax'] == 1){ // PND1 ////////////////////////////////////////////////////////
				...
				if($row['calc_method'] == 'acm'){
					...
					$gross_year = $fixed_actual + $acm_tax + $variable_income_this_month;
					$taxable_year = $gross_year - $total_tax_deductions;
					
					$total_tax_year = $acm_tax; //$acm_tax_variable;
					$tax_previous = $prev_tax;
					$tax_remaining = $acm_tax - $tax_previous; //$acm_tax_variable - $tax_previous;
			...
			//var_dump($gross_income_month);
			if($row['calc_tax'] == 3){ // PND3 /////////////////////////////////////////////////////
				$gross_year = 0;
				$taxable_year = 0;
				$tax_this_month = $gross_income_month * $pr_settings['wht'] / 100;
				$tax_month = $tax_this_month;
				$tax_next_month = 0;
			}
			//var_dump($tax_this_month);
			if($row['calc_tax'] == 0){ // NOTAX ////////////////////////////////////////////////////
				$tax_this_month = 0;
				$tax_month = 0;
				$tax_next_month = 0;
			}

			$tax_month = $tax_month + $modify_tax;

			$tax_this_month = round($tax_this_month,2);
			$tax_month = round($tax_month,2);
			$tax_next_month = round($tax_next_month,2);
		
		$msg .= '<b><br><br>Tax Summary ('.strtoupper($row['calc_method']).')</b>';	
	...
	// PAYROLL RESULT ////////////////////////////////////////////////////////////
			//$tot_deductions = $pvf_employee + $psf_employee + $sso_emp + $tot_deduct_after + $tax_month + $row['legal_deductions'] + $row['advance'] + $fix_deduct_before + $var_deduct_before + $tot_absence;
			$tot_deductions = $pvf_employee + $psf_employee + $sso_emp + $tot_deduct_after + $tax_month + $row['legal_deductions'] + $fix_deduct_before + $var_deduct_before + $tot_absence;
			...
			if($row['calc_base'] == 'gross'){
				...
				if($row['sso_by']){
					$gross_income_month += $sso_emp;
					$sso_by_company = $sso_emp;
					$net_month += $sso_emp;
				}
			}else{
				...
				/*if($row['calc_base'] == 'net'){
					$gross_income_month = $gross_income_month - $tax_by_company;
				}*/
			}
			
			
			/*$atot = $net_month + $tax_this_month;
			$stot = round($net_month,2) + round($tax_this_month,2);*/
			/*var_dump($net_month);
			var_dump($tax_this_month);
			var_dump(round($net_month,2));
			var_dump(round($tax_this_month,2));
			var_dump($atot);
			var_dump($stot);*/
			/*if($atot > $stot){
				$diff = $atot - $stot;
		...
		//var_dump($gross_year);	
	// PAYROLL OVERVIEW ////////////////////////////////////////////////////////////////
			//$taxable['calc_method'] = $row['calc_method'];
		...
		//if($tot_var_all == '4950'){
			//$netincomeNew = ($this_salary + $tot_var_all) - $tot_deductions;
			if(is_numeric( $gross_income_month ) && floor( $gross_income_month ) != $gross_income_month){
				$netincomeNew = floor($net_month*100)/100;
			}else{
				$netincomeNew = $net_month;
			}

			//echo $netincomeNew1 = $net_month;
			//echo $netincomeNew = floor($net_month*100)/100;
			//echo '<br>'. $gross_income_month;
			//$tot_deductionsNew = floor($tot_deductions*100)/100;
			//echo $msg; exit;
		//}
