...
<div class="A4form" style="width:1000px;height:730px; padding:30px 30px 100px; background:#fff url(../images/forms/sso_th.png?<?=time()?>) no-repeat; background-size:100%">
	...
	<form id="sso_form">	
		...
		<div class="field13" style="top:26.2mm;left:47mm"><?=$edata["th_compname"]?></div>
			
		<div class="field12" style="top:40.4mm;left:51mm"><?=$sso_codes[$_SESSION['rego']['gov_branch']]['line1_th']?></div>
		<div class="field12" style="top:46.6mm;left:15mm"><?=$sso_codes[$_SESSION['rego']['gov_branch']]['line2_th']?></div>
		
		<div class="field12" style="top:54mm;left:35.6mm"><?=$sso_codes[$_SESSION['rego']['gov_branch']]['postal_th']?></div>

		<div class="field12" style="top:54mm;left:60.4mm"><?=$edata["comp_phone"]?></div>
		<div class="field12" style="top:54mm;left:95.6mm"><?=$edata["comp_fax"]?></div>
		...
		<div class="field12" style="top:104mm;left:21mm;"><?=$chars?></div>
		<div class="field12n" style="top:110mm;right:146.6mm"><?=$emps?></div>
		
		<div class="field11" style="top:128.6mm;left:13.6mm">
			<label><input checked type="radio" name="rr" value="1" class="checkbox notxt"><span></span></label>
		</div>
		<div class="field12" style="top:128.6mm;left:71mm"><?=$pages?></div>
		
		<div class="field11" style="top:134.2mm;left:13.6mm">
			<label><input disabled type="radio" name="rr" value="2" class="checkbox notxt"><span></span></label>
		</div>

		<div class="field11" style="top:140.0mm;left:13.6mm">
			<label><input type="radio" name="rr" value="3" class="checkbox notxt"><span></span></label>
		</div>
		
		<div class="field11" style="top:146.0mm;left:13.6mm">
			<label><input type="radio" name="rr" value="4" class="checkbox notxt"><span></span></label>
		</div>
		<div class="field12" style="top:145.2mm;left:26.6mm">
			<input style="width:100px" name="other" type="text" value="" />
		</div>
		
		<div class="field12 center" style="top:154mm;left:65mm;width:190px"><input placeholder="<?=$lng['Name']?>" class="formName" name="form_name" style="width:190px; text-align:center" type="text" value="<?=$name_position['name']?>"></div>
		<div class="field12" style="top:159.6mm;left:69mm"><input class="formPosition" name="form_position" style="width:200px" type="text" value="<?=$name_position['position']?>"></div>
		
		<? if($edata['digi_signature'] == 1 && !empty($edata['dig_signature'])){ ?>
		<div class="field" style="top:145mm;left:67mm"><img width="165px" src="<?=ROOT.$edata['dig_signature'].'?'.time()?>" /></div>
		<? }if($edata['digi_stamp'] == 1 && !empty($edata['dig_stamp'])){ ?>
		<div class="field" style="top:153mm;left:22mm"><img width="80px" src="<?=ROOT.$edata['dig_stamp'].'?'.time()?>" /></div>
		<? } ?>
	
		<div class="field12 formDay" style="top:166.6mm;left:74mm"><?=$_SESSION['rego']['formdate']['d']?></div>
		<div class="field12 formMonth" style="top:166.6mm;left:88mm"><?=$_SESSION['rego']['formdate']['m']?></div>
		<div class="field12 formYear" style="top:166.6mm;left:117mm"><?=$_SESSION['rego']['formdate']['thy']?></div>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$(document).on("click", ".print", function(e) {
				...
				data += '&' + $(this).data('type');
				window.open(ROOT+'payroll/print/print_sso_form_th.php?'+data, '_blank')
			});
